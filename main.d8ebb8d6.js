...
var labelPrototype = document.createElement('label');
  ...
  for (_iterator.s(); !(_step = _iterator.n()).done;) {
    ...
    try {
      for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
        ...
        var label = labelPrototype.cloneNode();
        label.className = 'field-label';
        label.htmlFor = input.id;
        label.innerText = splitCamelCase(input.name);
        var inputContainer = input.closest('div');
        inputContainer.prepend(label);
        var placeholder = splitCamelCase(input.name);
